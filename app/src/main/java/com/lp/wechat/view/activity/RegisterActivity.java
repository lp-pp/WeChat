...
import com.lp.wechat.MainActivity;
import com.lp.wechat.R;
import com.lp.wechat.WcApp;
import com.lp.wechat.common.DES;
...
public class RegisterActivity extends BaseActivity implements OnClickListener {
	...
	private void getRegister() {
		...
		mNetClient.post(Constants.RegistURL, params, new BaseJsonRes() {
	...
	private void getCode() {
		...
		mNetClient.post(Constants.SendCodeURL, params,
				new JsonHttpResponseHandler() {
					...
					public void onSuccess(JSONObject response) {
						...
						try {
							...
							if (result == null) {
								Utils.showLongToast(WcApp.getInstance(),
										Constants.NET_ERROR);
							} else if (result.equals("Y")) {
								String str = response.getString("value");
								Utils.showLongToast(WcApp.getInstance(), str);
		...
		public void onTextChanged(CharSequence cs, int start, int before,
			...
			if (phone.length() == 11) {
				if (Utils.isMobileNO(phone)) {
					...
					Utils.showLongToast(mContext, "请输入正确的手机号码！");
				}
			} else {
